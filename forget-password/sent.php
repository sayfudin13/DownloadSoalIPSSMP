<?php
  if (!isset($_GET['e'])) {
    ...
    $email = $_GET['e'];
    $query = mysqli_query($link, "SELECT * FROM data where email = '$email' and status = 1 and changepass <> 'none'");
...
<body>
  <input id="reset-password-resend-email" type="hidden" name="email" value="<?=$email?>">
